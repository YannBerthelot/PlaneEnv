...
# real to beat -- and the docs say how good each one is.
...
Approaching 1.0. The reward contract, the physics contracts, the baselines and
the measurement protocol are settled; what remains is publishing learned-policy
results and hosting the documentation.

Known gaps are recorded rather than hidden -- five strict `xfail` cases, each
carrying the measurement that produced it, so a defect that gets fixed fails the
suite until its annotation is updated.
