...
_FIGURE8_PATH_XFAIL = (
    ...
    "error was never measured. For a long time nothing else saw it either: the "
    "benchmark episode was 200 steps against a 221 s lap, so every other test "
    "scored less than one lap with the aircraft starting exactly on the path, "
    "and a controller that simply flew straight ahead looked correct for all of "
    "it. That episode is now 800 steps -- three laps -- so the shortfall is "
    "visible to any test that measures cross-track error, not only to this one. "
